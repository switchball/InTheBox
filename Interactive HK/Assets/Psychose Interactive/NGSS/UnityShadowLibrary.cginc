...
inline fixed UnitySampleShadowmap (float4 shadowCoord)
{
	...
	float diskRadius = 0.015 * (1.0 - _LightShadowData.r);
...
inline half UnitySampleShadowmap (float3 vec)
{
	...
	//#if defined (NGSS_USE_PCSS_FILTER)
		...
		*/
		...
		float shadowTest = shadow;	
		//return shadowTest;	
		
		if (shadowTest >= 15.99)//If all pixels are lit early bail out
			return 1.0;
		else if (shadowTest <= 1.0)//If all pixels are shadowed early bail out
			return Shadow_Coef;
		#else
		shadow = 0.0;
		#endif
		...
		return shadow / (16.0 + Samplers_Count);
