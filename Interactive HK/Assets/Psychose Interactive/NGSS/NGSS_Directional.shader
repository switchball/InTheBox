...
//#define NGSS_USE_RECEIVER_PLANE_BIAS//If enabled you better disable light Bias and Normal Bias entirelly. Otherwise you are biasing shadows twice, in light and in shader
#if defined(NGSS_USE_RECEIVER_PLANE_BIAS)
#define NGSS_RECEIVER_PLANE_BIAS 0.1
#endif

//#define NGSS_USE_EARLY_BAILOUT_OPTIMIZATION_DIR

#if (UNITY_VERSION < 20171)
#define NGSS_USE_CASCADE_BLENDING
#define NGSS_CASCADE_BLEND_DISTANCE 0.25
#elif !defined(SHADOWS_SPLIT_SPHERES)
#define NGSS_USE_CASCADE_BLENDING
#define NGSS_CASCADE_BLEND_DISTANCE 0.25
#endif
...
//#define NGSS_POISSON_SAMPLING_NOISE_DIR 1.0 //Example: 0.0 gives 100 % Banding and 10.0 gives 100 % Noise. If value is 0 you better disable Poisson sampling completly, saves few instructions
//#define NGSS_USE_DITHERING_DIR //Convert small poisson samplers into squared shaped texels

//#define NGSS_USE_BIAS_FADE_DIR //Help with bias problems but can leads to some noisy artefacts if Early Bailout Optimization is enabled. Requires PCSS in order to work.
#if defined(NGSS_USE_BIAS_FADE_DIR)
#define NGSS_BIAS_FADE_DIR 0.0001
#endif

#if defined(NGSS_PCSS_FILTER_DIR)
//#define NGSS_PCSS_GLOBAL_SOFTNESS 0.01
//#define NGSS_PCSS_FILTER_DIR_MIN 0.05 //Close to blocker (If 0.0 == Hard Shadows)
//#define NGSS_PCSS_FILTER_DIR_MAX 0.25 //Far from blocker (If 1.0 == Soft Shadows)
#endif
...
//#define DIR_POISSON_16 //Mobile
//#define DIR_POISSON_25 //Consoles VR
//#define DIR_POISSON_32 //Desktop VR / Consoles
//#define DIR_POISSON_64 //Desktop / Consoles
//#define DIR_POISSON_128 //High Desktop (Coming soon)

#if defined(DIR_POISSON_64) || defined(DIR_POISSON_128)
...
#if (SHADER_TARGET >= 35) && (UNITY_VERSION >= 20171)
#define NGSS_INLINE_SAMPLER
#endif

#if defined(NGSS_INLINE_SAMPLER)
//DEFINE ALL INLINE SAMPLERS HERE
#endif
/*
//Not currently needed
#ifndef NGSS_USE_STATIC_NOISE
...
//#if defined(NGSS_PCSS_FILTER_DIR)
sampler2D_float NGSS_DirectionalRawDepth;
//UNITY_DECLARE_TEX2D(NGSS_DirectionalRawDepth);
float4 NGSS_DirectionalRawDepth_TexelSize;
//#endif
...
{
	return DirRandValue01(seed) * NGSS_POISSON_SAMPLING_NOISE_DIR;// clamp(NGSS_POISSON_SAMPLING_NOISE_DIR, 0.0, 10.0);
}
...
#if defined(NGSS_USE_RECEIVER_PLANE_BIAS)
		float shadowMapDepth = tex2Dlod(NGSS_DirectionalRawDepth, float4(UnityCombineShadowcoordComponents(uv.xy, offset, depthBiased, receiverPlaneDepthBias).xyz, 0.0)).r;
		//float shadowMapDepth = UNITY_SAMPLE_TEX2D(NGSS_DirectionalRawDepth, UnityCombineShadowcoordComponents(uv.xy, offset, depthBiased, receiverPlaneDepthBias).xy).r;
#else
		float shadowMapDepth = tex2Dlod(NGSS_DirectionalRawDepth, float4(uv + offset, depthBiased, 0.0)).r;
		//float shadowMapDepth = UNITY_SAMPLE_TEX2D(NGSS_DirectionalRawDepth, uv + offset).r;
#endif
...
float PCF_FilterDir(float2 uv, float receiver, float diskRadius, float3 receiverPlaneDepthBias, float c, float s, float Sampler_Number)
{
	...
	for (int i = 0; i < Sampler_Number; i++)
	{
		float2 offset = DirPoissonDisks[i] * diskRadius;
...
#if defined(NGSS_USE_BIAS_FADE_DIR) && defined(NGSS_PCSS_FILTER_DIR)
	#if defined(NGSS_USE_RECEIVER_PLANE_BIAS)
		float value = saturate((depthBiased - tex2Dlod(NGSS_DirectionalRawDepth, float4(UnityCombineShadowcoordComponents(uv.xy, offset, depthBiased, receiverPlaneDepthBias), 0.0)).r) / NGSS_BIAS_FADE_DIR);
	#else
		float value = saturate((depthBiased - tex2Dlod(NGSS_DirectionalRawDepth, float4(uv + offset, depthBiased, 0.0)).r) / NGSS_BIAS_FADE_DIR);
	#endif
#else
	#if defined(NGSS_USE_RECEIVER_PLANE_BIAS)
...
float PCF_FilterTestDir(float2 uv, float receiver, float diskRadius, float3 receiverPlaneDepthBias, float c, float s)
{
	float sum = 0.0f;
...
float NGSS_Main(float4 coords, float3 receiverPlaneDepthBias, float c, float s)
{
	...
	//float4 rotation = tex2D(unity_RandomRotation16, screenPos.xy);// *2.f - 1.f;
	//float2 theta = rotation.xy * 100000000.0 * ngss_pi2;//ngss_pi 3.141592653589793238462 | ngss_pi2 6.283185307179586476924
	//float c = cos(theta.x);
	//float s = sin(theta.y);

#if defined(NGSS_USE_EARLY_BAILOUT_OPTIMIZATION_DIR) && !defined(DIR_POISSON_16)
	float shadowTest = PCF_FilterTestDir(uv, receiver, shadowSoftness * NGSS_PCSS_FILTER_DIR_MAX, receiverPlaneDepthBias, c, s);
	//return shadowTest;
	if (shadowTest == 1.0)//If all pixels are lit early bail out
		return 1.0;
	else if (shadowTest == 0.0)//If all pixels are shadowed early bail out
		return lerp(_LightShadowData.r, 1.0f, 0.0);
#endif
	...
	//else if (blockerResults.y == Sampler_Number)//There are 100% occluders so early out (this saves filtering)
		//return 0.0;//UNITY_SAMPLE_SHADOW(_ShadowMapTexture, float4(uv.xy, receiver, 0));
...
fixed4 frag_ngss(v2f i) : SV_Target
{
    UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(i); // required for sampling the correct slice of the shadow map render texture array
...
#if defined (NGSS_USE_DITHERING_DIR)
	float4 rotation = tex2D(unity_RandomRotation16, i.uv.zw * _ScreenParams.xy * 0.125) * 2.f - 1.f;
	//float4 rotation = tex2D(NGSS_NoiseTexture, i.uv.xy * _ScreenParams.xy * 0.1) * 2.f - 1.f;
	float randAngle = DirRandAngle(rotation.xyz);
#else
...
#endif
*/
	//float shadow = UnitySampleShadowmap_PCF5x5GaussianCustom(coord, receiverPlaneDepthBias);
	...
	half alpha = dot(z4 * cascadeWeights, half4(1, 1, 1, 1));//Upcoming NGSS cascade sphere blending bae :D
#else
	half4 z4 = (float4(vpos.z, vpos.z, vpos.z, vpos.z) - _LightSplitsNear) / (_LightSplitsFar - _LightSplitsNear);
	...
	if (alpha > 1.0 - NGSS_CASCADE_BLEND_DISTANCE)
	{
		...
		//float shadowNextCascade = UnitySampleShadowmap_PCF5x5GaussianCustom(coord, receiverPlaneDepthBias);
		float shadowNextCascade = NGSS_Main(coord, receiverPlaneDepthBias, c, s);
...
// Requires SM3.0 GPU. Compatible with: DX9 (SM3.0), DX11, DX12, PS4, XB1, GLES3.0, Metal, Vulkan.
// If having texture interpolators or instruction limit errors please set shader model target to 4.0 or 5.0. Otherwise leave it at 3.0

//PCF FILTERING
SubShader
{
	Tags { "ShadowmapFilter" = "HardShadow" }//PCF FILTERING
	Pass
	{
...
SubShader
{
	Tags { "ShadowmapFilter" = "HardShadow_FORCE_INV_PROJECTION_IN_PS" }//PCF FILTERING
	Pass
	{
		ZWrite Off ZTest Always Cull Off
...
Subshader
{
	...
	Pass
	{
		...
		#pragma multi_compile DIR_POISSON_16 DIR_POISSON_25 DIR_POISSON_32 DIR_POISSON_64
		#pragma multi_compile_shadowcollector		
		#pragma target 3.0
...
Subshader
{
	Tags {"ShadowmapFilter" = "PCF_5x5"}//Unity 5.6
	Pass
	{
		ZWrite Off ZTest Always Cull Off
...
Subshader
{
	Tags{ "ShadowmapFilter" = "PCF_5x5_FORCE_INV_PROJECTION_IN_PS" }//Unity 5.6	
	Pass
	{
		ZWrite Off ZTest Always Cull Off
